...
export function systemPrompt(now: Date = new Date()): string {
  ...
  return [
    ...
    '- "scheduled", "reminders", "what am I being reminded about" mean scheduled true — the tasks that notify the user. That is not a due date, so do not answer it with due_from/due_to, and pass no status filter unless the user gave one.',
    ...
    `Every ${SEARCH_TASKS} result already says what a task is scheduled for: \`schedule\` is null when nothing notifies about it, and otherwise gives kind "once" or "recurring", the cron, and the next firing. Read it rather than guessing from due dates, and say plainly when a task has no schedule.`,
