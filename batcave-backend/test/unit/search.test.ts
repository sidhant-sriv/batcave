...
describe('buildSearchQuery', () => {
  ...
  it('requires every term to match, in the title or the description', () => {
    ...
    const groups = sql.match(/\(title LIKE \? ESCAPE '\\' OR description LIKE \? ESCAPE '\\'\)/g);
  ...
  it('orders dated tasks first, then by date, priority and age', () => {
    ...
    expect(sql.indexOf('due_date ASC')).toBeLessThan(sql.indexOf('CASE priority'));
    expect(sql.indexOf('CASE priority')).toBeLessThan(sql.indexOf('created_at ASC'));
  });
